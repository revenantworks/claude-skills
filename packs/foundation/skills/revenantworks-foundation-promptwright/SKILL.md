...
**This entry also covers a handoff or task brief written for a future session or agent to execute, whatever it's called when asked for** — "give me a prompt to hand off", "write a brief for the next session", "what should I paste into a fresh session to do X" — even when what ships is a multi-step brief rather than a single-shot prompt (observation #0071). The trigger is *something is being handed to a future executor*, not *a prompt is being built*: run this entry's four steps and fold the recommendation into the brief (a `Model:` line near the top is enough — no phase ladder, no prompt block). Skipping this because the ask didn't look like "build me a prompt" is exactly the gap that produced #0071 — a real handoff shipped with no tier pick at all until asked about separately. Writing the handoff's own content — the state, the steps, the exclusions — is not this skill's job; see the description's line to resumewright (session state) or to plain writing (a forward brief with nothing to score).
