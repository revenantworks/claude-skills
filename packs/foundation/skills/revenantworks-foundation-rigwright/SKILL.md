...
**A claim about the current state of work is the wrong layer (observation #0080).** A gate open, a milestone reached, a run paused, a wave stopped — all of these change, and `CLAUDE.md` is the copy every session reads, the one that is injected into every dispatched subagent, and the one nobody rereads for staleness. Standing config names the single status file that owns the state, by path, and says nothing about what it currently says; a fresher copy of the claim is the same defect again. Where two files both read as current, the reader follows whichever their instructions name first, and the older one wins often enough to matter — so the audit scores a restated status claim under **rot**, and the fix is a pointer, never an update.

**Where two accounts share a rig, the permission layer says which surfaces each may drive (observation #0088).** That an account is logged in is not that it is active, and not that it may perform the next action: a push can succeed over one credential while opening the pull request needs another, and switching between them is a persistence change a sandboxed or dispatched session is refused — correctly. So the config records the routing per identity: which remote, which repo, and which action (push · PR · merge · release) each account may drive, and which steps are therefore controller- or human-run. Written down, a brief or a build can be checked against it; unwritten, it is discovered by denial halfway through the step it blocks.
