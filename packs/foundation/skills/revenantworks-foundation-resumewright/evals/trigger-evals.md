...
Provenance: authored at member version 1.0.0, 2026-09-11, alongside the member's first build. **Re-anchored to v1.0.2, 2026-09-13 — provenance only, nothing executed here.** The 1.0.2 change lands the weekly review's doctrine edits (#0045); the `description` field is byte-identical, so the routing surface these judge did not move. **Extended to v1.0.3, 2026-09-14 (observation #0071):** the `description` gained "hand off"/"handoff" as explicit trigger phrasing plus the forward-task-brief exclusion, so the routing surface moved and a full cold re-judge is **OWED, not yet performed** — record here rather than skip silently. #15 added (should-not: a forward task brief for new work, which reads like a handoff but has no session state to verify). 16 → **17**; the split moves 7/7 → **7/8** pending the re-judge. **Extended to v1.0.4, 2026-09-14 (observation #0072, owner ruling):** the `description` widened to own the forward task-brief shape too, so #15 flips should-not → should (renumbered **#8** in the should-fire table) and calls promptwright's Entry — Model internally instead of routing there. 17 stays 17; the split moves 7/8 → **8/7**. The re-judge owed since 1.0.3 is still owed, now against the 1.0.4 text. **Re-anchored to v1.1.0, 2026-09-14:** the `description` moved again — trimmed from 1321 to 994 characters to clear the 1024 house ceiling and a YAML parse failure (an unquoted `Entry: Model`), extended to name the paste-ready starter prompt, and cut of the stale claim that dispatchwright's tiering is promptwright's (#0073). Every trigger phrase is kept verbatim, and no query, expected value, or count moved; still 17, 8/7. The cold re-judge owed since 1.0.3 is still owed, now against the 1.1.0 text. **Re-anchored to v1.1.1, 2026-09-14:** that re-judge was PERFORMED. Against 1.1.0, one fresh blind judge scored **13 / 15**: #14 judged SHOULD (a regression 1.1.0's fallback wording introduced) and #12 AMBIGUOUS against rigwright. The `description` was fixed with one exclusion sentence (dispatchwright for fan-out resume, rigwright for the hook, task-observer for no filesystem), and a second fresh judge scored 1.1.1 at **15 / 15** (`RESULTS.md`). No query, expected value, or count moved; still 17, 8/7.
**Re-anchored to v1.0.1, 2026-09-11 — provenance only, nothing executed here:** `SOURCES.md` **Re-anchored to v1.1.2, 2026-09-22:** provenance only, nothing executed here: the 2026-09-20 task-observer batch was installed (doctrine and references only). The `description` is byte-identical, so no query, expected value, or count moved.
