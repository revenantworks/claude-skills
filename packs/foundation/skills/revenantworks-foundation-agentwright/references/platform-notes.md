...
**Creation path decides what a scheduled job can be given later** *(observed 2026-09-12 in use, not vendor-documented — confirm on the next refresh)*. A job created as a Cowork task carries no `sources` field, so a repo read cannot be attached to it afterwards; getting one means recreating the job on the API-created routine path, which is a different object with a different home, not a setting. So a fix whose only mechanism is "attach a real source" is a creation-type change, and emit raises it as the step 1 boundary question rather than performing it: the surface an unattended agent was created on is rarely changeable in place, and the change is the owner's to make.
