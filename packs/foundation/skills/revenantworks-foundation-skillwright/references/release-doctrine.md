...
1. **Member work lands** — each touched member with its own dated CHANGELOG entry, eval provenance re-anchored in the same commit — **and only on a green gate** (observation #0077). A change never merges while `build.py --check` or the tool suite is red. If main is already red when the change is ready, its first commit makes main green, or it waits; "the failure was already there" is not a reason to merge. A gate is informative only while its baseline is green: the first merge on red spends it, because every later failure then has an innocent explanation. Four PRs once merged on a red check in one day, twelve errors piled up behind them, and the next release first had to repair five members. A branch protection rule that requires the check makes this structural; that setting is the repo owner's to turn on, so recommend it rather than assume it.
