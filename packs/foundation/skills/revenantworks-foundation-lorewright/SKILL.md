...
**A summarising tool is a second author between you and the source** (added 2026-09-20, task-observer observation #0066). Two rules, both binding on any figure that reaches a table. *(a)* A number read out of a **search-result summary** is **[unverified]** while the page it cites can still be fetched — fetch the page, use its own figure, and name the index, dataset, or version the page states, because snippets routinely quote an older edition of a score the live page has since restated (52 and 24 in the snippets, 34 and 14 on the pages, same run). *(b)* Where one page covers several entities side by side and the fetch tool **summarises** rather than returns it, read each entity's **own** page for every deciding cell. Where only the combined page exists, cross-check one figure that must agree across two fetches, and **drop any figure that appears identically for two different entities** — a summariser that moves a cell between columns hands back a plausible number attached to the wrong candidate, and nothing in the number shows the swap. Check that a figure stayed attached to the right entity, not only that it is present.

**A tool's behaviour is settled by reading the tool** (added 2026-09-20, observation #0061). Where a verdict turns on *why* a piece of software behaves as it does and that software can be read or imported from here, read or instrument it — a verdict that stops at black-box trials is graded **inference, not observation**, and says so in the confidence line. A black-box control tests one guess per run and returns one bit; an instrumented run returns every decision the tool made, with offsets. Measured once at fourteen controls to zero information against one instrumented run to root cause. A control that comes back clean also owes the same question an empty result does — *could this control have fired at all?* — and **where a probe was placed is part of what it tests**: text appended at the end of a file exercises the end-of-input path, which is routinely special-cased, and is not the same probe as the identical text mid-file. A refuted hypothesis is evidence only if the control could have confirmed it.

**A computed figure publishes its computation** (added 2026-09-11, task-observer observation #0037). Where a verdict or playbook rests on figures this run *derived* — a normalised cost, a ratio, a weighted score, a projection — the product carries **the formula and the inputs**, not only the result, and where a script produced them, the script ships with the artefact so a reader can re-derive rather than trust. Grade a derived figure by its inputs: a computation over two `[unverified]` cells is `[unverified]`, however exact the arithmetic looks.

The failure this prevents is not lying. A document that says "every value below is measured" and ships no instrument has published **trust, not evidence**, and the two age into each other: one such definition held about 160 figures, of which one had been asserted from a shape rather than measured, one was true when written and invalidated by a later unrelated change nobody connected to it, and one was contradicted three lines further down. Care had not been missing. The dangerous residue is the figure that was correct when written — it looks exactly like the newest one. **An artefact that publishes derived values needs something that re-derives them, or its oldest numbers are its least reliable.** Where the artefact is versioned, re-run that re-derivation at every version, not only when a figure is edited.
